...
import { BadRequestError, IAuthDocument, IEmailMessageDetails, lowerCase } from '@aprydatko/jobber-shared';
...
export async function resendEmail(req: Request, res: Response): Promise<void> {
  ...
  const checkIfUserExists: IAuthDocument = await getUserByEmail(lowerCase(email));
